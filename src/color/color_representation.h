/// color_representation.h

/// this is the basic class of color representation
/// Four Colors Representations Only
/// TODO: refactor by DimensionOneVector

#pragma once

#include "../basic/global.h"
#include "../basic/notation.h"
#include <vector>

namespace PlanarGraphColoring {

class ColorRepresentation {
public:
  inline std::vector<size_t> getVertices(const std::vector<size_t>& colors) const {
    /// no matter colors  contain duplicate
    std::vector<size_t> res;
    for (size_t v = 0; v < this->size(); ++v) {
      bool find = false;
      for (const auto& color : colors) {
        if (this->get(v) == color) {
          find = true;
          break;
        }/// if
      }/// for color
      if (find) {
        res.emplace_back(v);
      }/// if find
    }/// for v
    return res;
  }/// getVertices
  virtual size_t size() const = 0;
  virtual bool empty() const = 0;
  virtual size_t get(const size_t index) const = 0;
  inline virtual size_t getMax() const {
    size_t res = 0;
    for (size_t i = 0; i < this->size(); ++i) {
      if (this->get(i) == UNDEF_VERTEX) continue;/// WARNING
      res = std::max(res, this->get(i));
    }/// for
    return res;
  }/// getMax
  inline virtual std::vector<size_t> getVector() const {
    size_t max = this->getMax();
    std::vector<size_t> res(max + 1, 0);
    for (size_t i = 0; i < this->size(); ++i) {
      res[i] = this->get(i);
    }/// for
    return res;
  }/// getVector
  virtual void set(const size_t index, const size_t color) = 0;
  virtual void reset(const size_t index) = 0;
  virtual void show(std::ostream& cout) const = 0;
  inline virtual bool valid() const {
    bool res = true;
    for (size_t i = 0; i < this->size(); ++i) {
      if (this->get(i) != UNDEF_COLOR) continue;
      res = false;
      break;
    }/// for
    return res;
  }/// valid
  inline virtual bool equal(const ColorRepresentation& rhs) const {
    return this->getVector() == rhs.getVector();
  }/// equal
  inline void inverse(const VI& vertices, const II& colors) {
    for (const auto& vertex : vertices) {
      if (this->get(vertex) == colors.first) {
        this->set(vertex, colors.second);
      } else if (this->get(vertex) == colors.second) {
        this->set(vertex, colors.first);
      }
    }/// for
  }/// inverse
  virtual ~ColorRepresentation() = default;
};/// class ColorRepresentation

}/// namespace PlanarGraphColoring
