...
test_file="_test_edge_inducer.cxx"
parameter=""
if [ $# -ge 1 ]; then
  parameter=$1
fi
...
g++ -Wall -std=c++17 -o ${obj_file} -cpp ${test_file} edge_inducer.cxx ${basic_src_path}graph.cxx ${basic_src_path}global.cxx ${topology_src_path}birkhoff_diamond.cxx ${topology_src_path}planar_graph.cxx ${topology_src_path}ring.cxx ${algebra_src_path}symmetry.cxx
